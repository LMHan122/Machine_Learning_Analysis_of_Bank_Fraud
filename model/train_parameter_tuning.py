...
def train_hyperperameters():
    ...
    artifact = wandb.use_artifact(
        "lhan122-student/credit_card_fraud/train_data:latest", type="dataset"
    )
    ...
    precision, recall, f1, auc = train_evaluate_model(
        xgb_model, X_train, X_test, y_train, y_test, run
    )

    # logging metrics using wandb.log and not just logger
    wandb.log({"precision": precision, "recall": recall, "f1": f1, "auc": auc})
